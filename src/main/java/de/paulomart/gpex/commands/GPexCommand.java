	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("gpex")){
			...
			if (args.length == 2 && args[0].equalsIgnoreCase("list")){
				...
				OfflinePlayer offlinePlayerTarget = BukkitUtils.getPlayer(args[1]);
				if (offlinePlayerTarget == null){
					sender.sendMessage("§cCant find player");
					return true;
				}
				
				UUID uuid = offlinePlayerTarget.getUniqueId();
				...
				if (result.getBasePlayerPermissions() != null){
					sender.sendMessage("§b§lData §afor "+offlinePlayerTarget.getName());
			...
			if (args.length >= 3 && args[0].equalsIgnoreCase("set")){				
				...
				try {
					...
					if (offlinePlayerTarget.isOnline())
						gpex.getPermissionManager().getPermissible(offlinePlayerTarget.getPlayer()).recalculatePermissions(true);
					
					sender.sendMessage("§aData updated for "+offlinePlayerTarget.getName());
					return true;
				} catch (Exception e) {
					sender.sendMessage(e.toString());
